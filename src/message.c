/*
** $Id: message.c,v 1.34 2006/06/07 09:21:28 krishnap Exp $
**
** Matthew Allen
** description: 
*/
...
void *message_init (void *chstate, int port)
{
    ...
    if (pthread_create (&tid, &mg->attr, network_activate, (void *) state) !=
	0)
	{
	    log_message (state->log, LOG_ERROR, "pthread_create: %s",
			 strerror (errno));
	    return (NULL);
	}
...
/**
 ** message_reciever:
 ** is called by the message_recieved func. this function will find the proper 
...
void message_recieved (void *chstate, char *data, int size)
{
    ...
    job_submit (msgglob->jobq, message_reciever, (void *) jargs, 0);
		  ...
		  Bool retry)
{
	...
	for (i = 0; i < MAX_RETRY && ret != 1; i++)
	    {
		ret = network_send (state, host, data, size, msgprop->ack);
		if (ret != 1)
		    {
			log_message (state->log, LOG_WARN,
				     "network_send(transmission#%i) to %s:%d failed ,success_avg: %f\n",
				     i, host->name, host->port,
				     host->success_avg);
