/*
**
** $Id: chimera.c,v 1.46 2006/06/07 09:21:28 krishnap Exp $
**
** Matthew Allen
** description: 
*/
 ...
 ** initiates a separate thread the constantly checks to see if the leafset members 
...
int chimera_check_leafset_init (ChimeraState * state)
{
	...
	0)
	{
	    log_message (state->log, LOG_ERROR, "(CHIMERA)pthread_create: %s",
			 strerror (errno));
	    goto out;
	}
...
void chimera_join_denied (ChimeraState * state, Message * message)
{
    ...
    log_message (state->log, LOG_WARN, "JOIN request rejected from %s:%d !\n",
		 host->name, host->port);

    sleep (GRACEPERIOD);

    log_message (state->log, LOG_WARN, "Re-sending JOIN message to %s:%d !\n",
