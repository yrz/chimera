		     ...
		     ChimeraHost ** added)
{
	...
	{
	    key_distance (state->log, &dif1, &routeglob->me->key, &p[i]->key);
	    key_distance (state->log, &dif2, &routeglob->me->key, &host->key);
	    while ((i < size) && (key_comp (&dif1, &dif2) <= 0))
		{
		    ...
		    if (i < size)
			{
			    key_distance (state->log, &dif1,
					  &routeglob->me->key, &p[i]->key);
			    key_distance (state->log, &dif2,
					  &routeglob->me->key, &host->key);
			}
		}
